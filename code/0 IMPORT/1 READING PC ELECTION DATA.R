# nolint: assignment_linter, line_length_linter, spaces_inside_linter, commas_linter, 
# commented_code_linter, infix_spaces_linter, object_name_linter, object_usage_linter
rm(list = ls())
...
# and can potentially be useful. Given that AC can directly be linked to
# Districts and they do not cross districts, this effectively allows us to
# break the PC votes among districts if there are more than one. 
...
dd = pc_dt %>% 
  dplyr::select(pid, assembly_no, contested) %>% 
  group_by(pid) %>% 
  mutate(cmax = max(contested)) %>% 
...
dd = pc_dt %>% 
  ...
  group_by(pid) %>% 
  mutate(tt = any(recontest)) %>% 
  ungroup() %>% 
  dplyr::select(-recontest) %>% 
  distinct()
...
incumbent_re_election_13_14 = el_pc_delim %>%
  ...
  left_join(el_pc_delim %>% 
              filter(assembly_no == 13) %>% 
              dplyr::select(pc_uid, pc_type) %>% 
              distinct(), 
            by = "pc_uid") %>%
  group_by(pc_type) %>%
  summarise(
