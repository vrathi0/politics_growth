...
exp_df=exp_df %>% mutate(start_date=as.Date(start_date), 
  ...
  #mutate(home_state=standardize_state_names(place_of_domicile))# %>%
...
exp_df <- exp_df %>% 
  group_by(id) %>% 
  mutate(
    max_lvl = ifelse(all(is.na(reference_value)), NA, max(reference_value, na.rm = T)),
    min_lvl = ifelse(all(is.na(reference_value)), NA, min(reference_value, na.rm = T)+1),
    post_no = ifelse(is.na(reference_value), NA, abs(max_lvl - reference_value) + 1),
    record_per_id = n(),
    ) %>% 
  ungroup()

# See #1 in the note/.txt file for issues in the raw data at this point
...
ias_det=profile_df %>% dplyr::select(id, allotment_year, date_of_birth, 
                                   ...
                                   mother_tongue,languages_known,
                                   last_education_qualification, 
                                   last_education_subject, 
                                   last_level,last_office,
  ...
  filter(id_dup==1) %>% dplyr::select(-id_dup) %>% ungroup() %>%
  rename(home_state=place_of_domicile) %>%
  mutate(home_state=standardize_state_names(home_state)) %>% 
  rename(recruit_type=source_of_recruitment) %>% 
  left_join(lang_st_class,
            by=c("home_state"="st_name")) #Joining with `by = join_by(languages_known)`
...
ias_df=ias_df %>% 
  ...
  mutate(first_start_date = if_else(reference_value==max_lvl, 
                                    start_date, NA_Date_), 
         last_start_date2=if_else(reference_value==min_lvl, 
                                 start_date, NA_Date_)) %>%
  fill(first_start_date, .direction = "up") %>% 
  fill(last_start_date2, .direction = "down") %>%
  ungroup() %>% 
...
# 2 [RECONSIDERED ] REMOVING SOME "BAD" RECORDS -----------------------------------------------------------

# FEB 7 NOT REMOVING ANY
...
# ias_df=ias_df %>% # filter(!(id %in% bad_id1$id)) %>% 
...
ias_df=ias_df %>% filter(!(id %in% bad_id4$id ))



# 3 CREATING CAREER LEVELS----------------------------------------------------------
...
level_centre_df1=ias_dr_df %>% 
  mutate(centre_dum=if_else(grepl("Centre", organisation),1,0)) %>%
  group_by(level_eq, id) %>%
  mutate(centre_deput=max(centre_dum, na.rm=T)) %>% ungroup() %>% 
  group_by(level_eq) %>%
  summarise(level_centre=mean(centre_deput, na.rm=T), 
            level_post_no=quantile(post_no,0.5, na.rm=T), 
            level_mis_p10=quantile(month_in_service,0.1, na.rm=T),
            level_mis_p50=quantile(month_in_service,0.5, na.rm=T),
            level_mis_p90=quantile(month_in_service,0.9, na.rm=T),
            count=n())  %>% filter(count>100) %>%
  arrange(level_post_no)
...
ias_exp=ias_exp %>% 
  mutate(post_id=paste(id, reference_value, sep="-"))

ias_exp=ias_exp %>% 
  rename(cohort=batch_yr)

ias_exp=ias_exp %>% 
  rename(cadre_st=cadre) %>% 
  mutate(cadre_st=standardize_state_names(cadre_st)) %>% 
  group_by(id) %>% 
  mutate(jnr_post_count= sum(grade <= 12, na.rm = TRUE)) %>%   # Count postings with grade <= 12
  ungroup() %>% 
  mutate(state_cs=if_else(recruit_type=="By Promotion from State Civil Service",
                          1, 0))#
 
ias_state_exp=ias_exp#
# 4 SAVING MAIN VERSION ----------------------------------------------------------------


qsave(ias_state_exp, "data/2 CLEAN/tcpd_ias+state_exp.qs") # USE THIS FOR INPUT 


dt=ias_state_exp %>% 
  dplyr::select(id,name, cadre_st, post_no,max_lvl, min_lvl,
                designation, level_eq, grade,
                office, organisation, field_of_experience, 
                category_of_experience, centre_dum,
                first_start_date, date_of_joining, 
                cohort, recruit_type, 
                state_cs, duration, 
                last_education_subject,last_education_qualification, 
                first_centre, jnr_post_count,
                id_centre_deput) %>%  distinct()
 


# 5 ALT VERSION: MERGED AT DISTRICT LEVEL-----------------------------------------

# here I am keeping the district merged version in one place
# since I am using it in multiple places, better to have one version

collector_df=ias_state_exp %>% 
  mutate(designation=tolower(designation)) %>%
  #filter((grepl("collector",designation))) %>% 
  filter(!is.na(office)) %>% 
  mutate(office=tolower(office)) %>% 
  #filter(!grepl(remove_ptr,office)) %>% 
  mutate(office=str_trim(str_remove_all(office, "\\s*\\(.*?\\)"))) %>% 
  distinct() %>% 
  arrange(cadre_st, office) %>% 
  rename(ias_name=name) 

# Merging with district names
dist_collector_df=collector_df %>% 
  inner_join(ac_pc_dist_delim_map %>% 
               dplyr::select(dist_name, st_name) %>% 
               distinct(), 
             by=c("office"="dist_name")) %>% 
  mutate(cadre_st_mismatch=(cadre_st== st_name))
 

qsave(dist_collector_df, "data/2 CLEAN/IAS +StateCS DATA MERGED WITH DELIM MAP_AT DIST LEVEL.qs")
